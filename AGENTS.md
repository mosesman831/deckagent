...
6. OAuth MUST be GitHub PKCE. NO exceptions.
7. All config in `~/.deckagent/config.json`. NO env vars.
8. Daemon runs as user-space process. NO sudo, NO root.
9. Log to `~/.deckagent/logs/`. 10MB rotation. Keep 5 files.
10. EVERY tool gets a smoke test. NO untested code.
...
ChatGPT/Claude/Gemini Web
    ↓  HTTPS POST /mcp (Streamable HTTP, OAuth Bearer token)
...
- GitHub OAuth PKCE at /auth/github and /auth/callback
...
| GitHub OAuth PKCE | ChatGPT's custom connector requires OAuth. GitHub is universal. |
