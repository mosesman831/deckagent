...
const matchingUrls = [
  ...
  "https://chat.qwen.ai/api/v1/chat",
  "https://qwen.ai/api/chat/completions",
  "https://qwen.ai/api/v1/chat",
  "https://chat.qwenlm.ai/api/conversations",
  "https://chat.qwenlm.ai/api/v1/chat/completions",
  "https://qwenlm.ai/api/chat/completions",
  "https://qwenlm.ai/api/v1/chat",
  "https://tongyi.aliyun.com/qianwen/api/messages",
  "https://tongyi.aliyun.com/qianwen/api/chat",
];

for (const url of matchingUrls) {
  test(`matches ${url}`, () => assert(qwenAdapter.match(url), `expected ${url} to match`));
}

const nonMatchingUrls = [
  "https://chat.openai.com/api/chat/completions",
  "https://api.deepseek.com/chat/completions",
  "https://qwen.com/api/chat/completions",
  "https://tongyi.aliyun.com/api/chat/completions",
  "https://chat.qwen.net/api/chat/completions",
  "http://chat.qwen.ai/api/chat/completions",
];

for (const url of nonMatchingUrls) {
  test(`does not match ${url}`, () => assert(!qwenAdapter.match(url), `expected ${url} to NOT match`));
}
...
test("injects system prompt into request without system message", () => {
  const req = makeRequest(JSON.stringify({ messages: [{ role: "user", content: "hello" }], model: "qwen-max" }));
  const modified = qwenAdapter.transformRequest?.(req) as InterceptedRequest;
...
test("prepends tool prompt before existing system message", () => {
  const req = makeRequest(JSON.stringify({ messages: [{ role: "system", content: "Be helpful." }], model: "qwen-max" }));
  const modified = qwenAdapter.transformRequest?.(req) as InterceptedRequest;
  const parsed = JSON.parse(modified.body) as { messages: Array<{ role: string; content: string }> };
  assertEqual(parsed.messages[0]?.role, "system");
...
test("does not duplicate tool prompt when system message already contains DeckAgent", () => {
  const req = makeRequest(JSON.stringify({ messages: [{ role: "system", content: "DeckAgent tools available." }], model: "qwen-max" }));
  const modified = qwenAdapter.transformRequest?.(req) as InterceptedRequest;
  const parsed = JSON.parse(modified.body) as { messages: Array<{ role: string; content: string }> };
...
test("handles request with array body format", () => {
  const req = makeRequest(JSON.stringify([{ messages: [{ role: "user", content: "hello" }], model: "qwen-max" }]));
  const modified = qwenAdapter.transformRequest?.(req) as InterceptedRequest;
  // Should not throw; array bodies are unsupported for injection
...
test("handles request with no messages array", () => {
  const req = makeRequest(JSON.stringify({ model: "qwen-max", input: "hello" }));
  const modified = qwenAdapter.transformRequest?.(req) as InterceptedRequest;
  assertEqual(modified.body, req.body);
});

test("handles request with null messages", () => {
  const req = makeRequest(JSON.stringify({ messages: null, model: "qwen-max" }));
  const modified = qwenAdapter.transformRequest?.(req) as InterceptedRequest;
  assertEqual(modified.body, req.body);
});
...
test("transformResponse returns response unchanged", () => {
  ...
  const transformed = qwenAdapter.transformResponse?.(req, res);
  assertEqual(transformed, res);
});

// --- extractToolCalls: Qwen native response ---

test("extracts tool call from Qwen native non-streaming response", () => {
  const body = JSON.stringify({ output: { text: '<<<TOOL>>>{"name":"read_file","args":{"path":"/etc/hosts"}}<<<END>>>' } });
  const calls = extractToolCalls(body);
  assertToolCall(calls, 0, { name: "read_file", args: { path: "/etc/hosts" } });
});

test("extracts tool call using output.content path", () => {
  const body = JSON.stringify({ output: { content: '<<<TOOL>>>{"name":"write_file","args":{"path":"/tmp/a.txt","content":"hi"}}<<<END>>>' } });
  const calls = extractToolCalls(body);
  assertToolCall(calls, 0, { name: "write_file", args: { path: "/tmp/a.txt", content: "hi" } });
});

test("extracts tool call using top-level text", () => {
  const body = JSON.stringify({ text: '<<<TOOL>>>{"name":"search_files","args":{"path":"/home","pattern":"*.ts"}}<<<END>>>' });
  const calls = extractToolCalls(body);
  assertToolCall(calls, 0, { name: "search_files", args: { path: "/home", pattern: "*.ts" } });
});

test("extracts tool call using top-level content", () => {
  const body = JSON.stringify({ content: '<<<TOOL>>>{"name":"list_directory","args":{"path":"/"}}<<<END>>>' });
  const calls = extractToolCalls(body);
  assertToolCall(calls, 0, { name: "list_directory", args: { path: "/" } });
});

// --- extractToolCalls: OpenAI-compatible response ---
...
test("extracts tool call from OpenAI choices.0.text", () => {
...
test("extracts tool call from Qwen native SSE chunks split mid-tool", () => {
  const sse = [
    'data: {"output":{"text":"I will use a tool"}}',
    'data: {"output":{"text":". <<<TOOL>>>"}}',
    'data: {"output":{"text":"{\\"name\\":\\"browser_navigate\\",\\"args\\":{\\"url\\":\\"https://example.com\\"}}<<<END>>>"}}',
    "data: [DONE]",
  ].join("\n\n");
...
test("extracts tool call from OpenAI SSE chunks split mid-tool", () => {
  const sse = [
    'data: {"choices":[{"delta":{"content":"<<<TOOL>>>"}}]}',
    'data: {"choices":[{"delta":{"content":"{\\"name\\":\\"browser_screenshot\\",\\"args\\":{}}<<<END>>>"}}]}',
    "data: [DONE]",
  ].join("\n\n");
  const calls = extractToolCalls(sse);
  assertToolCall(calls, 0, { name: "browser_screenshot", args: {} });
});
...
test("handles SSE with mixed valid and invalid data lines", () => {
  const sse = [
    ...
    'data: {"output":{"text":"ok"}}',
    "data:",
    'data: {"output":{"text":"<<<TOOL>>>{\\"name\\":\\"get_environment\\",\\"args\\":{}}<<<END>>>"}}',
  ].join("\n");
  const calls = extractToolCalls(sse);
  assertToolCall(calls, 0, { name: "get_environment", args: {} });
});
...
test("handles SSE without trailing [DONE]", () => {
  const sse = 'data: {"output":{"text":"<<<TOOL>>>{\\"name\\":\\"read_file\\",\\"args\\":{\\"path\\":\\"/x\\"}}<<<END>>>"}}';
  const calls = extractToolCalls(sse);
...
test("returns null for non-streaming response without tool markers", () => {
  const calls = extractToolCalls(JSON.stringify({ output: { text: "Hello there" } }));
  assertEqual(calls, null);
});
...
test("appends tool result to messages", () => {
  const body = JSON.stringify({ messages: [{ role: "user", content: "hello" }], model: "qwen-max" });
