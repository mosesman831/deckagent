...
function extractTextFromOpenAiChunk(chunk: unknown): string {
  return (
    ...
    getString(chunk, "choices", "0", "text") ??
    ""
  );
}
...
export const kimiAdapter: ChatAdapter = {
  name: "kimi",
  hostPattern: /^(https?:\/\/)?([^/]+\.)?(kimi\.com|moonshot\.ai)(\/.*)?$/i,

  match(url: string): boolean {
    try {
      const parsed = new URL(url);
      const host = parsed.hostname.toLowerCase();
      if (host === "kimi.com" || host.endsWith(".kimi.com") || host === "moonshot.ai" || host.endsWith(".moonshot.ai")) {
        return true;
      }
      // Bail out for non-Kimi hosts so generic endpoints like OpenAI don't match.
      return false;
    } catch {
      // Not a parseable URL — fall through to endpoint pattern check.
    }
