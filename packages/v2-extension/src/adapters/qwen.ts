...
function extractTextFromQwenChunk(chunk: unknown): string {
  return (
    getString(chunk, "output", "text") ??
    getString(chunk, "output", "content") ??
...
function extractTextFromResponse(data: unknown): string {
  return (
    ...
    getString(data, "choices", "0", "message", "content") ??
    getString(data, "choices", "0", "text") ??
    ""
  );
}
  ...
  if (content.trim().startsWith("data:")) {
    ...
    for (const rawLine of content.split("\n")) {
      ...
      try {
        ...
        if (parsed && typeof parsed === "object") {
          accumulated.push(extractTextFromQwenChunk(parsed) || extractTextFromOpenAiChunk(parsed));
        }
      } catch {
...
export const qwenAdapter: ChatAdapter = {
  name: "qwen",
  hostPattern: /^https:\/\/(chat\.(qwen\.ai|qwenlm\.ai)|qwen(\.ai|lm\.ai)|tongyi\.aliyun\.com\/qianwen)/i,
